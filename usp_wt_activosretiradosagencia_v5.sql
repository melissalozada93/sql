...
-- Calendario desde el 2022
	...
	----Extraer data socios 2021
	DROP TABLE IF EXISTS #Z_DW_SOCIO
	SELECT * 
	INTO #Z_DW_SOCIO
	FROM Z_DW_DATOSSOCIO WITH (NOLOCK)
	WHERE dw_fechaCarga='2021-12-31' AND SITUACION=1
	AND COMPRACARTERA='N' and CODIGODEPENDENCIADESCRI<>'Originacion'


	----Extraer data personas 2021
...
-- Datos de socios activos 2021

	DROP TABLE IF EXISTS #DW_PERSONA_SOCIO_2021
	SELECT DISTINCT
		DS.CODIGOSOCIO,
		...
		'2022-01-01'FECHAINGRESOCOOP,
		DS.CODIGOPERSONA,
		DS.AGENCIADESCRI,
		DS.SITUACION,
		CONVERT(DATE, NULL) AS FECHAAPROBACIONREN,
		'ACTIVO' AS ESTADO
	INTO #DW_PERSONA_SOCIO_2021
	FROM #Z_DW_SOCIO DS 
	INNER JOIN #Z_DW_PERSONA P  ON P.CIP = DS.CODIGOSOCIO 
	LEFT JOIN DW_OBSERVACION O WITH (NOLOCK) ON P.CODIGOPERSONA=O.CODIGOPERSONA AND O.TIPOOBSERVACION=12
	AND P.CIP IS NOT NULL and DS.CODIGODEPENDENCIADESCRI<>'Originacion'
...
-- Datos de socios ingresados
	...
	LEFT JOIN DW_OBSERVACION O WITH (NOLOCK) ON P.CODIGOPERSONA=O.CODIGOPERSONA AND O.TIPOOBSERVACION=12
	WHERE P.FECHAINGRESOCOOP >= '2022-01-01'  AND DS.SITUACION<>2 ---Situacion 2 -= anulado
	AND DS.dw_fechaCarga=@fecha AND P.CIP IS NOT NULL
	AND COMPRACARTERA='N' and DS.CODIGODEPENDENCIADESCRI<>'Originacion'
...
WHERE AGENCIADESCRI<>'COMPRA CARTERA'
...
-- Mostrar resultados finales ordenados
	...
	ORDER BY 
		FECHA,
			...
			select * from WT_ACTRETAGENCIA_RESUMEN
