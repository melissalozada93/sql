...
-- Variables
DECLARE @fecha DATE = CAST(GETDATE() - 1 AS DATE)
...
WHERE 
    ...
    AND pr.dw_fechacarga = CAST(GETDATE() - 1 AS DATE)
    AND p.DW_FECHACARGA = CAST(GETDATE() - 1 AS DATE)
    AND pa.DW_FECHACARGA = CAST(GETDATE() - 1 AS DATE)
    AND per.DW_FECHACARGA = CAST(GETDATE() - 1 AS DATE)
	AND SP.DW_FECHACARGA = CAST(GETDATE() - 1 AS DATE)
    AND i.TBLCODTAB = 10
