USE [DWCOOPAC]
GO
/****** Object:  StoredProcedure [dbo].[usp_dwt_dinerofresco_dpf]    Script Date: 30/04/2024 15:14:18 ******/
--SET ANSI_NULLS ON
--GO
--SET QUOTED_IDENTIFIER ON
--GO

--ALTER procedure [dbo].[usp_dwt_dinerofresco_dpf]
--as
---- Declarar la fecha de la tabla FECHAMAESTRA
DECLARE @FECHA DATE;
SET @FECHA =(SELECT FECHA FROM [DWCOOPAC].dbo.ST_FECHAMAESTRA);

--SET @FECHA ='2024-05-18'-- (SELECT FECHA FROM [DWCOOPAC].dbo.ST_FECHAMAESTRA);
...
DECLARE @TC DECIMAL(15,3) = (select promedio from DW_XTIPOCAMBIO where fecha = @fecha and codigoTipoCambio = 3)
...
AND FECHAUSUARIO >= DATEADD(DAY, -8, @FECHA)



DROP TABLE IF EXISTS #NUMEROCUENTA
SELECT CODIGO,NUMEROCUENTA, [dbo].[UFN_NUMEROCUENTA](RIGHT(OBSERVACION,( CHARINDEX(' ', REVERSE(
	   OBSERVACION)))-1))[NUMEROCUENTA2] ,OBSERVACION
INTO #NUMEROCUENTA
...
ON A.NUMEROCUENTA=B.NUMEROCUENTA2 AND A.CODIGO<>B.CODIGO AND A.FECHAUSUARIO >= DATEADD(DAY, -7, @FECHA)
...
-----Actualizar Hora de deposito y fecha apertura de movimientos de manera cruzada
UPDATE A
...
DELETE FROM #MONEDACRUZADA WHERE CODIGO IN(
SELECT DISTINCT CODIGO FROM #MONEDACRUZADA WHERE  FECHAUSUARIO=fechaapertura) 
    ...
    SELECT 
        *,
        IIF(ROW_NUMBER() OVER(PARTITION BY CODCIPPERSONA,FECHAAPERTURA,MONEDA
			ORDER BY FECHAAPERTURA,[HORAMOVIMIENTO] DESC)>1,0,1) AS RowNum
...
SELECT R9.*,
	...
	DFM=   IIF(R9.SOCIO_UNICO=1,ISNULL(C.IMPORTE,0),0) --------Cuenta Mancomunada
	INTO #DINEROFRESCO
FROM 
