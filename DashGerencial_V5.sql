USE [DWCOOPAC]
GO
/****** Object:  StoredProcedure [dbo].[usp_dash_excolaboradores]    Script Date: 09/02/2024 15:55:52 ******/
SET ANSI_NULLS ON
GO
...
CREATE procedure [dbo].[usp_dash_indicadores_gerencia]
as
set nocount on --
set xact_abort on
	begin transaction
...
--        select getdate(),'Ejecucion Exitosa del Dashboard Colaboradores',null, 'OK'
	...
	begin catch
		...
		insert into DWCOOPAC.dbo.LOG_DASHBOARD (logFecha,logMensaje,logError,logEstado) 
        select getdate(),'ERROR en la ejecucion del Dashboard Indicadores Gerencia', @error_message, 'ERROR'
