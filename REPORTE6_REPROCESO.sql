...
-----------DWT_CUENTAMOVIMIENTO----------------------------------------
  ...
  INNER JOIN DW_CUENTACORRIENTE cc on a.NUMEROCUENTA = cc.NUMEROCUENTA
  ...
  AND a.CODIGOAGENCIA not in(9,12) AND A.dw_fechaCarga=@FECHA AND cc.dw_fechaCarga=@FECHA
...
---------DWT_PRESTAMO_PAGOS----------------------------------------
  ...
  INNER JOIN DW_PRESTAMO p ON pp.CODIGOSOLICITUD = p.CODIGOSOLICITUD 
  INNER JOIN DW_SOLICITUDPRESTAMO sp ON sp.CODIGOSOLICITUD = p.CODIGOSOLICITUD 
  ...
  and pp.CODIGOAGENCIACAJA not in(9,12)  AND  pp.dw_fechaCarga=@FECHA and p.DW_FECHACARGA=@FECHA
