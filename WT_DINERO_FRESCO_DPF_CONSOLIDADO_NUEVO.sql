USE [DWCOOPAC]
GO
--/****** Object:  StoredProcedure [dbo].[usp_dwt_dinerofresco_dpf_consolidado]    Script Date: 12/04/2024 16:17:13 ******/
--SET ANSI_NULLS ON
--GO
--SET QUOTED_IDENTIFIER ON
--GO

--ALTER procedure [dbo].[usp_dwt_dinerofresco_dpf_consolidado]
--as
-- Declarar la fecha de la tabla FECHAMAESTRA
DECLARE @FECHA DATE;
--SET @FECHA = (SELECT FECHA FROM [DWCOOPAC].dbo.ST_FECHAMAESTRA);
SET @FECHA ='2024-04-16'
...
----Cruce por moneda, fecha y hora
DROP TABLE IF EXISTS #TEMP_REPORTE6
SELECT 
	A.CODIGO,
	A.NUMEROCUENTA,
	A.MONEDA,
	...
	A.HORA,
	A.FORMAPAGO2,
	NFORMAPAGO=IIF(A.FORMAPAGO2='Transf. Interna',2,1),
	A.IMPORTE,
	B.HORADEPOSITO,
	B.FECHAAPERTURA,
	ROW_NUMBER() OVER(PARTITION BY A.CODIGO, A.MONEDA, A.DINERO_FRESCO, A.FECHAUSUARIO, A.HORA, A.FORMAPAGO2, A.IMPORTE ORDER BY (SELECT NULL)) AS RowNum
...
LEFT JOIN
	#REPORTE9 B 
	ON A.CODIGO=B.CODCIPPERSONA AND A.MONEDA=B.MONEDA  AND CAST(A.FECHAUSUARIO AS DATETIME) + CAST(A.HORA AS DATETIME)  BETWEEN DATEADD(DAY, -8, CAST(B.FECHAAPERTURA AS DATETIME) + CAST(B.HoraDeposito AS DATETIME)) 
    AND CAST(B.FECHAAPERTURA AS DATETIME) + CAST(B.HoraDeposito AS DATETIME)
WHERE A.FECHAUSUARIO >= DATEADD(DAY, -8, @FECHA)
	AND A.TIPOMOVIMIENTO='ENTRADA'
	AND A.PRODUCTO='AHV'

----Cruce por fecha y hora en movimientos de moneda cruzada 

DROP TABLE IF EXISTS #TEMP_REPORTE6_2
SELECT 
	A.CODIGO,
	A.NUMEROCUENTA,
	A.MONEDA,
...
INTO #TEMP_REPORTE6_2
FROM
	WT_REPORTE6 A 
LEFT JOIN
	#REPORTE9 B 
	ON A.CODIGO=B.CODCIPPERSONA AND CAST(A.FECHAUSUARIO AS DATETIME) + CAST(A.HORA AS DATETIME)  BETWEEN DATEADD(DAY, -8, CAST(B.FECHAAPERTURA AS DATETIME) + CAST(B.HoraDeposito AS DATETIME)) 
    AND CAST(B.FECHAAPERTURA AS DATETIME) + CAST(B.HoraDeposito AS DATETIME)
WHERE A.FECHAUSUARIO >= DATEADD(DAY, -7, @FECHA)
	AND A.TIPOMOVIMIENTO='ENTRADA'
	AND A.PRODUCTO='AHV'



-----Actualizar Hora de deposito y fecha apertura de movimientos de moneda cruzada
UPDATE A
SET HoraDeposito=b.HoraDeposito,fechaapertura=b.fechaapertura
--SELECT *
FROM #TEMP_REPORTE6 A INNER JOIN #TEMP_REPORTE6_2  B ON A.CODIGO=B.CODIGO
AND CAST(A.FECHAUSUARIO AS DATETIME) + CAST(A.HORA AS DATETIME)  BETWEEN DATEADD(DAY, -8, CAST(B.FECHAAPERTURA AS DATETIME) + CAST(B.HoraDeposito AS DATETIME)) 
    AND CAST(B.FECHAAPERTURA AS DATETIME) + CAST(B.HoraDeposito AS DATETIME) AND A.HoraDeposito IS NULL



---------------Eliminar movimientos considerados y usados en cuentas mancomunadas----------------------------------------------------
DELETE FROM #TEMP_REPORTE6 WHERE CONCAT(CODIGO,NUMEROCUENTA,MONEDA,IMPORTE,FECHAUSUARIO,HORA) IN(
SELECT CONCAT(C.CODIGO_MOV,C.NUMEROCUENTA,C.MONEDA,C.IMPORTE,C.FECHAUSUARIO,C.HORA) FROM #CUENTAMANCOMUNADA C INNER JOIN #REPORTE9 R9 
ON R9.codcippersona=C.CODIGO_DPF AND R9.moneda=C.MONEDA AND R9.fechaapertura=C.FECHAUSUARIO AND R9.HoraDeposito>=C.HORA)



---------------Agrupar campos para el reporte 6----------------------------------------------------
...
FROM
	#TEMP_REPORTE6
WHERE  CAST(FECHAAPERTURA AS DATETIME) + CAST(HORADEPOSITO AS DATETIME)> CAST(FECHAUSUARIO AS DATETIME) + CAST(HORA AS DATETIME) AND RowNum=1
GROUP BY 	CODIGO,
	MONEDA,
	DINERO_FRESCO,
	FECHAUSUARIO,
...
---------------------Extraer movimientos del reporte 9 de días anteriores ------------------------------------------------------------	
DROP TABLE IF EXISTS #REPORTE9MC
SELECT *,
SOCIO_UNICO=IIF(ROW_NUMBER() OVER(PARTITION BY CODCIPPERSONA,FECHAAPERTURA,MONEDA
			ORDER BY FECHAAPERTURA, HoraDeposito ASC)>1,0,1)
...
FROM 
	WT_REPORTE9
WHERE   FECHAAPERTURA>= DATEADD(DAY, -8, @FECHA)
--YEAR(FECHAAPERTURA)=YEAR(@FECHA)
AND CONVERT(DATE,ISNULL(F_CANCELALIQUIDA,GETDATE()))>fechaapertura


DROP TABLE IF EXISTS #MONEDAREPORTE6
SELECT DISTINCT CODIGO,MONEDA,FECHAUSUARIO
INTO #MONEDAREPORTE6
FROM #REPORTE6 WHERE CODIGO IN (SELECT  DISTINCT codcippersona FROM #REPORTE9MC)


DROP TABLE IF EXISTS #FALTAREPORTE6
SELECT  codcippersona,moneda 
INTO #FALTAREPORTE6
FROM #REPORTE9MC WHERE CONCAT(codcippersona,moneda) NOT IN (SELECT CONCAT(CODIGO,MONEDA) FROM #MONEDAREPORTE6)


DROP TABLE IF EXISTS #SOBRAREPORTE61
SELECT  A.CODIGO,A.MONEDA ,B.MONEDA [MONEDAAPERTURA],B.fechaapertura,B.HoraDeposito
INTO #SOBRAREPORTE61
FROM #MONEDAREPORTE6 A
LEFT JOIN #REPORTE9MC B ON A.CODIGO=B.codcippersona 
WHERE CONCAT(A.CODIGO,A.MONEDA) NOT IN (SELECT CONCAT(codcippersona,moneda ) FROM #REPORTE9MC)


DROP TABLE IF EXISTS #SOBRAREPORTE62
SELECT  A.CODIGO,A.MONEDA ,B.MONEDA [MONEDAAPERTURA],B.fechaapertura,B.HoraDeposito
INTO #SOBRAREPORTE62
FROM #MONEDAREPORTE6 A
LEFT JOIN #REPORTE9MC B ON A.CODIGO=B.codcippersona AND A.FECHAUSUARIO=B.fechaapertura
WHERE CONCAT(A.CODIGO,A.MONEDA,A.FECHAUSUARIO) NOT IN (SELECT CONCAT(codcippersona,moneda,fechaapertura ) FROM #REPORTE9MC)
...
FROM #SOBRAREPORTE61
UNION 
SELECT * FROM #SOBRAREPORTE62 WHERE CONCAT(CODIGO,MONEDA,fechaapertura) NOT IN (SELECT CONCAT(CODIGO,MONEDA,fechaapertura)  FROM #SOBRAREPORTE61 )AND MONEDAAPERTURA IS NOT NULL


DROP TABLE IF EXISTS #TEMP_MONEDACRUZADA
SELECT DISTINCT A.*,
IMPORTE2=IIF(MONEDAAPERTURA='D',IMPORTE/C.PROMEDIO,IMPORTE*C.PROMEDIO),
B.MONEDAAPERTURA,
B.FECHAAPERTURA
INTO #TEMP_MONEDACRUZADA
FROM #REPORTE6 A 
...
DELETE FROM #TEMP_MONEDACRUZADA WHERE CODIGO IN(
SELECT DISTINCT CODIGO FROM #TEMP_MONEDACRUZADA WHERE  FECHAUSUARIO=fechaapertura) 
AND FECHAUSUARIO<>fechaapertura

DELETE FROM #TEMP_MONEDACRUZADA WHERE IMPORTE2 IS NULL 
...
SELECT 
	...
	MONEDA,
	DINERO_FRESCO,
	MAX(FECHAUSUARIO)FECHAUSUARIO,
	MAX(HORA)HORA,
	SUM(IMPORTE)IMPORTE,
	SUM(IMPORTE2)IMPORTE2,
	MONEDAAPERTURA,
	fechaapertura
INTO #MONEDACRUZADA
FROM #TEMP_MONEDACRUZADA 
GROUP BY CODIGO,
	MONEDA,
	DINERO_FRESCO,
	MONEDAAPERTURA,
    ...
    SELECT 
        *,
        IIF(ROW_NUMBER() OVER(PARTITION BY CODCIPPERSONA,FECHAAPERTURA,MONEDA
			ORDER BY [HORAMOVIMIENTO]DESC,HORADEPOSITO ASC)>1,0,1) AS RowNum
...
-----Eliminar movmientos de dinero fresco que no se debe considerar	
DELETE FROM #MONEDACRUZADA WHERE IMPORTE2 IS NULL 
...
-----Agregando movimientos de moneda cruzada al reporte 6---------------------
DROP TABLE IF EXISTS #NREPORTE6
SELECT A.*,
	NIMPORTE=ISNULL(B.IMPORTE2,A.IMPORTE),
	NMONEDA=ISNULL(B.MONEDAAPERTURA,A.MONEDA)
INTO #NREPORTE6
FROM #REPORTE6 A
LEFT JOIN #MONEDACRUZADA B
ON A.CODIGO=B.CODIGO AND A.MONEDA=B.MONEDA AND A.FECHAUSUARIO=B.FECHAUSUARIO AND A.HORA=B.HORA
...
SELECT 
    ...
    SUM(B.NIMPORTE) AS IMPORTE
INTO #TEMP_DINEROFRESCO_7DIAS
FROM 
    #SIN_DINERO_FRESCO A
INNER JOIN 
    #NREPORTE6 B ON A.codcippersona = B.codigo AND A.moneda = B.NMONEDA
WHERE 
...
ORDER BY 
	...
	FROM 
		(SELECT * FROM #DINEROFRESCO2 WHERE (DF1+MCfinal)>0) A
	INNER JOIN 
		#NREPORTE6 B ON A.codcippersona = B.codigo AND A.moneda = B.NMONEDA
	WHERE 
		CAST(B.FECHAUSUARIO AS DATETIME) + CAST(B.HORA AS DATETIME) BETWEEN DATEADD(DAY, -8, CAST(A.FECHAAPERTURA AS DATETIME) + CAST(A.HoraDeposito AS DATETIME)) 
	...
	WHERE B.SALDO>0 AND B.fechaapertura<>B.FECHAUSUARIO

	

	

	-----4.3 Eliminar los registros duplicados , cuando hay más de una apertura en el rango de 7 días---------------------
	...
	-----4.4 Calcular el saldo restante del dinero fresco de la primera apertura del rango de 7 días---------------------
...
SELECT D.*,
	DF =  IIF(D.SOCIO_UNICO=1,
			CASE WHEN ISNULL(D7.IMPORTE,D.DF1+D.MCfinal) >= D.MONTOAPERTURA THEN D.MONTOAPERTURA
				 WHEN D.MONTOAPERTURA >ISNULL(D7.IMPORTE,D.DF1+D.MCfinal) THEN ISNULL(D7.IMPORTE,D.DF1+D.MCfinal) ELSE 0 END,0),
	DNF = IIF(D.SOCIO_UNICO=1, 
			CASE WHEN ISNULL(D7.IMPORTE,D.DF1+D.MCfinal) >= D.MONTOAPERTURA THEN 0
				 WHEN D.MONTOAPERTURA >ISNULL(D7.IMPORTE,D.DF1+D.MCfinal) THEN  D.MONTOAPERTURA - ISNULL(D7.IMPORTE,D.DF1+D.MCfinal) ELSE 0 END,0)
...
SELECT A.*,
	 ...
	 DF_CONS_PR=CONVERT(FLOAT,0),
	 DNF_CONS_PR=CONVERT(FLOAT,0)
...
-- Calcular el prorrateo y actualizar la columna MONTO_PR
WITH SumaApertura AS (
    SELECT codcippersona, moneda, fechaapertura ,
	SUM(MONTOAPERTURA) AS SUMA_APERTURA,
	SUM(DF_CONS) AS DF_CONS,
	SUM(DF_CONS_PR) AS DF_CONS_PR,
	SUM(DNF_CONS) AS DNF_CONS,
	SUM(DNF_CONS_PR) AS DNF_CONS_PR
    FROM #DINEROFRESCOSOLES
    GROUP BY codcippersona, moneda, fechaapertura
)
UPDATE A
SET A.DF_CONS_PR = (A.montoinicial * S.DF_CONS) / S.SUMA_APERTURA, 
    A.DNF_CONS_PR = (A.montoinicial * S.DNF_CONS) / S.SUMA_APERTURA
FROM #DINEROFRESCOSOLES A
JOIN SumaApertura S
    ON A.codcippersona = S.codcippersona AND A.MONEDA = S.MONEDA AND A.fechaapertura=S.fechaapertura;
...
SELECT D.*,
	...
	CAMPAÑA = CASE WHEN D.TPERSON IN (
							...
							D.SUBPRODUCTO IN (
                                SELECT DISTINCT VARIABLE
                                FROM [dbo].[ST_CAMPANAS_DPF_DETALLE]
                                WHERE CAMPANA = C.CAMPANA AND CARACTERISTICA = 'SUBPRODUCTO' AND d.tasaintanualapert=VALOR3  AND D.MONTOINICIAL >= VALOR1 AND D.MONTOINICIAL<=VALOR2 )
	...
	INICIO_CAMAPAÑA= CASE WHEN D.TPERSON IN (
                                SELECT DISTINCT VARIABLE
                                FROM [dbo].[ST_CAMPANAS_DPF_DETALLE]
                                WHERE CAMPANA = C.CAMPANA AND CARACTERISTICA = 'TPERSON') AND
							D.SUBPRODUCTO IN (
...
LEFT JOIN DW_XTIPOCAMBIO B ON D.fechaapertura=B.fecha
WHERE B.codigoTipoCambio = 3
